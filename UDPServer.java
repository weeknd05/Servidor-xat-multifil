...
/**Aquest codi és un servidor de xarxa UDP que espera la connexió de clients. 
 * Després d'escoltar al port 7778, quan el servidor rep un paquet d'un client
 * que comença amb "HAL", es crea un nou fil per aquest client i es registra a la llista
 * de clients connectats. El servidor envia un missatge de resposta al client que indica
 * que ha estat autenticat. Aquest servidor té la capacitat de enviar missatges a tots
 * els clients connectats excepte a qui l'ha enviat.
 * En el mètode principal, es crea una nova instància de la classe UDPServer que 
 * s'encarrega de començar a escoltar els clients a través del mètode listenSocket(). 
 * A més, s'inicialitza una ArrayList anomenada clientsConnected per mantenir una llista
 * dels clients connectats al servidor. La classe ClientThread implementa la interfície
 * Runnable i s'utilitza per enviar missatges als clients connectats. El mètode sendMessageToAll()
 * envia un missatge a tots els clients connectats excepte al client que ha enviat el missatge.
 */
